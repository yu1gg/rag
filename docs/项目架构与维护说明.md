...
本项目当前是一套基于检索增强生成（RAG）的智能问答和文本摘要系统。它已经完成从旧版单体实验结构到前后端分离结构的重构，当前正式运行主体为：
...
| 前端工作台 | `frontend/src/views/WorkspaceView.vue` | 单工作台聊天界面，承载 QA / Summary |
...
5. 不要把 `/qa`、`/summary` 重新演化成两套互相复制的前端页面。
